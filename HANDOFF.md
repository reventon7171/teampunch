...
- **⚠️ เรื่องที่ลองแล้วไม่ได้ผล ก่อนจะเจอวิธีที่ใช้ได้จริง (กันคนถัดไปเสียเวลาซ้ำ)** — โปรเจกต์นี้ build ด้วย **Railpack** (Railway's ตัวใหม่) ไม่ใช่ Nixpacks แบบเดิม:
  1. `nixpacks.toml` field `aptPkgs` **หรือแม้แต่ `nixPkgs` ก็ใช้ไม่ได้กับ Railpack เวอร์ชันนี้เลย** — Railpack เมินไฟล์นี้ทั้งไฟล์เงียบๆ ไม่ error (build log ไม่มีคำว่า postgresql โผล่มาเลย) → `pg_dump` หายไปเฉยๆ = `ENOENT`
  2. ลอง env var `RAILPACK_DEPLOY_APT_PACKAGES=postgresql-client` — คราวนี้ `pg_dump` เจอจริง แต่ Debian trixie (deb13) ที่ Railpack ใช้มี apt package แค่เวอร์ชัน **17** ในขณะที่ Postgres ที่ Railway รันจริงคือ **v18.6** → `pg_dump: error: aborting because of server version mismatch` (pg_dump ปฏิเสธ dump จาก server ที่เมเจอร์เวอร์ชันใหม่กว่าตัวเองโดยตั้งใจ)
  3. ลอง `RAILPACK_PACKAGES=postgres@18` (ผ่าน mise) — ล้มเหลวเพราะเวอร์ชัน 18 ไม่มี prebuilt binary ต้อง compile จาก source แต่ build image ไม่มี `bison` (`configure: error: bison not found`)
- **✅ วิธีที่ใช้ได้จริง — build service นี้จาก Dockerfile ของตัวเอง แทน Railpack**: `backend/Dockerfile.backup` เริ่มจาก `postgres:18-bookworm` ตรงๆ (เวอร์ชันตรงกับ server เป๊ะ เพราะเป็น image เดียวกันสายพันธุ์เดียวกับที่ Railway ใช้รัน Postgres) แล้วติดตั้ง Node.js 22 ทับเข้าไปทาง NodeSource — วิธีนี้ไม่ต้องมานั่งไล่หาเวอร์ชัน package ให้ตรงกันอีกเลย เพราะ pg_dump มากับ image อยู่แล้ว
  - ตั้งค่า service `teampunch-backup` ให้ใช้ `dockerfilePath: "Dockerfile.backup"` แทน Railpack (ทำผ่าน Railway GraphQL API มิวเทชัน `serviceInstanceUpdate` — field `builder` enum มีแค่ `HEROKU/NIXPACKS/PAKETO/RAILPACK` ไม่มี "DOCKERFILE" แยก แต่การตั้ง `dockerfilePath` ก็เพียงพอให้ Railway สลับไปใช้ Dockerfile build เองอัตโนมัติ)
  - **ลบ `backend/nixpacks.toml` ทิ้งแล้ว** (ใช้ไม่ได้จริง เก็บไว้จะสับสน) และลบ env var `RAILPACK_PACKAGES`/`RAILPACK_DEPLOY_APT_PACKAGES` ออกจาก service `teampunch-backup` แล้ว (ไม่จำเป็นอีกต่อไปเมื่อ build ด้วย Dockerfile)
  - Gotcha ระหว่างทำ: `RUN npm ci` จะรัน postinstall (`prisma generate`) ทันที ต้อง `COPY prisma ./prisma` ให้เสร็จ**ก่อน** `RUN npm ci` ไม่งั้นจะเจอ "Could not find Prisma Schema" (`Dockerfile.backup` ตอนนี้เรียงลำดับถูกแล้ว)
- **✅ ทดสอบสำเร็จจริงแล้ว** (23 ส.ค. 2026) — กด "Run now" แล้วเช็ค R2 เจอไฟล์ `backups/teampunch-2026-08-23T09-05-58-384Z.dump` (37 KB) ขึ้นจริง สถานะ execution ขึ้น "COMPLETED" สีเขียว ไม่ต้องแก้อะไรเพิ่มแล้ว รอ cron รอบแรกตามตารางคืนนี้ตี 3 ได้เลย
- **Service ที่สร้างไว้แล้วใน Railway** (โปรเจกต์ `resilient-benevolence`): `teampunch-backup` — repo เดิม, Root Directory `backend`, Dockerfile `Dockerfile.backup`, Custom Start Command `npm run backup` (ตรงกับ Dockerfile's `CMD` อยู่แล้ว), Cron Schedule `0 20 * * *` (20:00 UTC = ตี 3 เวลาไทย ทุกคืน), copy env vars มาแล้ว (`DATABASE_URL`, `JWT_SECRET`, `R2_ACCOUNT_ID`, `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY`, `R2_BUCKET_NAME`)
- ทดสอบผ่านปุ่ม **"Run now"** ในแท็บ "Cron Runs" ของ service — ยืนยันด้วยการเช็คไฟล์ `backups/teampunch-....dump` ใน R2 bucket ตรงๆ ไม่ใช่แค่ดูสถานะ deploy เฉยๆ (ปุ่ม "Run now" ทำงานคนละอย่างจาก deploy ปกติ — deploy แค่ build image ไว้ ไม่ได้รัน start command จนกว่าจะถึงเวลา cron หรือกด Run now)
