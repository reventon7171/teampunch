...
Unit test ของ payroll engine: **72 ข้อ ผ่านหมด** (`backend/src/lib/payroll.test.ts`, ลดจาก 76 เพราะลบเทสต์ `isCheckInTooEarly` ที่ไม่มีฟังก์ชันให้ทดสอบแล้ว) — เป็นจุดที่ business logic ซับซ้อนที่สุดของระบบ ถ้าจะแก้อะไรเกี่ยวกับเงินเดือนอีก ให้รันเทสต์นี้ก่อน/หลังเสมอ
